      ...
      async.eachSeries(updateOperations, function(operation, callback) {
        async.auto({
          updatePreferences: function(callback) {
            if(!operation.preferences || !operation.preferences) {
              return callback();
            }
            var identity = {
              id: operation.id,
            ...
            brIdentity.setPreferences(req.user.identity, identity.id, identity.preferences, callback);
          },
          updateRoles: function(callback) {
            // Updating roles requires elevated permissions.  Attempt to update
            // roles first and if that fails then fail the entire update
            // operation.
          ...
          update: ['updatePreferences', 'updateRoles', function(callback, results) {
            var identity = {id: operation.id};

            // conditional values only set if preset
            if(operation.description) {
              identity.description = operation.description;
            }
            if(operation.image) {
              identity.image = operation.image;
            }
            if(operation.label) {
              identity.label = operation.label;
            }
            if(operation.sysGravatarType) {
              identity.sysGravatarType = operation.sysGravatarType;
            }
            if(operation.sysImageType) {
              identity.sysImageType = operation.sysImageType;
            }
            if(operation.sysPublic) {
              identity.sysPublic = operation.sysPublic;
            }
            if(operation.sysSigningKey) {
              identity.sysSigningKey = operation.sysSigningKey;
            }
            if(operation.url) {
              identity.url = operation.url;
            }
...
function updateIdentity(req, res, next) {
  async.auto({
    getId: getIdentityIdFromUrl.bind(null, req),
    checkId: ['getId', function(callback, results) {
      // check id matches
    ...
    updateRoles: ['checkId', function(callback, results) {
      ...
      if(!req.body.sysResourceRole) {
        return callback();
      }
      var identity = {
        id: results.getId,
        sysResourceRole: req.body.sysResourceRole
      };
      brIdentity.setRoles(req.user.identity, identity, callback);
    }],
