...
bedrock.events.on('bedrock-express.configure.routes', function(app) {
  ...
  app.post(basePath,
    ...
    function(req, res, next) {
      ...
      */
      ...
      createIdentity(
        req.user.identity, identity, function(err, result) {
        if(err) {
  ...
  app.get(basePath,
    ...
    function(req, res, next) {
      ...
      brIdentity.getAll(null, q, function(err, records) {
        if (err) {
          return next(err);
        }
        res.status(200).json(_.map(records, 'identity'));
      });
  ...
  // FIXME: this is a temporary endpoint used because the endpoint at
  // get /i/:id is not returning the full identity
  app.get('/id/:id', ensureAuthenticated, function(req, res, next) {
    var id = config.server.baseUri + basePath + '/' + req.params.id;
    brIdentity.get(req.user.identity, id, function(err, result) {
      if(err) {
        return next(err);
      }
  ...
  app.delete(idPath, ensureAuthenticated, function(req, res, next) {
    ...
    brIdentity.setStatus(
      req.user.identity, fullUrl, 'deleted', function(err) {
        if(err) {
          return next(err);
        }
        res.sendStatus(204);
