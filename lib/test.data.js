...
var baseIdPath = config.server.baseUri + config['identity-rest'].basePath;

// test account
config['identity-rest'].test = {};
config['identity-rest'].test.users = {};
config['identity-rest'].test.users.registered = {
  name: 'registereduser', role: 'identity.registered'
};
config['identity-rest'].test.users.admin = {name: 'adminuser', role: 'admin'};

config['identity-rest'].test.users.admin.keys = createKeyPair({
  userName: config['identity-rest'].test.users.admin.name,
...
config['identity-rest'].test.users.registered.keys = createKeyPair({
  userName: config['identity-rest'].test.users.registered.name,
  publicKey: '-----BEGIN PUBLIC KEY-----\n' +
...
for(var user in config['identity-rest'].test.users) {
  var currentUser =
  config['identity-rest'].identities.push(
    createIdentity(config['identity-rest'].test.users[user]));
  config['identity-rest'].keys.push(
    config['identity-rest'].test.users[user].keys);
}
...
function createIdentity(user) {
  var newIdentity = {
    id: baseIdPath + '/' + user.name,
    type: 'Identity',
    sysSlug: user.name,
    label: user.name,
    email: user.name + '@bedrock.dev',
