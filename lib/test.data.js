...
var baseIdPath = config.server.baseUri + config['identity-http'].basePath;

// test account
config['identity-http'].test = {};
config['identity-http'].test.users = {};
config['identity-http'].test.users.registered = {
  name: 'registereduser', role: 'identity.registered'
};
config['identity-http'].test.users.admin = {name: 'adminuser', role: 'admin'};

config['identity-http'].test.users.admin.keys = createKeyPair({
  userName: config['identity-http'].test.users.admin.name,
...
config['identity-http'].test.users.registered.keys = createKeyPair({
  userName: config['identity-http'].test.users.registered.name,
  publicKey: '-----BEGIN PUBLIC KEY-----\n' +
...
for(var user in config['identity-http'].test.users) {
  var currentUser =
  config['identity-http'].identities.push(
    createIdentity(config['identity-http'].test.users[user]));
  config['identity-http'].keys.push(
    config['identity-http'].test.users[user].keys);
}
